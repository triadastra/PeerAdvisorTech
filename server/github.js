...
export function installGithubRoutes(app, { requireAuth }) {
  ...
  app.use('/api/project-service', requireAuth, async (req, res) => {
    ...
    if (!/^\/(me|projects|tasks|credentials|github\/(repos|branches|device\/(start|poll))|tasks\/[a-f0-9-]{36}\/(join|submit|complete|archive|restore|delete))$/.test(req.path)) return res.status(404).json({ error: 'Unknown project action.' });
    try {
